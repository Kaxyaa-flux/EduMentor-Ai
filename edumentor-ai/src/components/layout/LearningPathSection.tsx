    ...
    <section className="py-24 bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <StaggerContainer className="relative">
          ...
          <div className="absolute left-8 top-8 bottom-8 w-px bg-gradient-to-b from-primary via-secondary to-transparent hidden md:block" />
            ...
            {steps.map((step, index) => (
              <ScrollReveal key={index} variant="slideUp" delay={index * 0.15}>
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6 relative group">
                  <div className="z-10 bg-background p-2 rounded-full hidden md:block">
                    <div className="h-12 w-12 rounded-full bg-card border-2 border-border group-hover:border-primary flex items-center justify-center transition-colors duration-300">
                      <span className="text-primary font-bold">{index + 1}</span>
                    </div>
                  </div>
                  
                  <div className="flex-1 bg-card/60 border border-border p-6 rounded-2xl group-hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 w-full md:w-auto">
                    <div className="flex items-center gap-3 mb-2">
                      <CheckCircle2 className="h-5 w-5 text-primary md:hidden" />
